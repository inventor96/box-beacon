...
class TemplateAddon extends Template {
	...
	/**
	 * Generates a hidden input with a new CSRF value.
	 *
	 * @param string $template
	 * @return string
	 */
	protected function csrf(string $template): string {
		return preg_replace('/{{\s*csrf\s*}}/i', '<?php echo $this->csrf(); ?>', $template);
	}
